...
int32_t PerformInit(void)
{
    ...
    /* OSTM2 is used as the run time stats counter. */
	CPG.STBCR3.BIT.MSTP34 = 0;      /*  OSTM2 */
    dummy_buf = CPG.STBCR3.BYTE;    /* (Dummy read) */

    /* Stop the counter. */
    OSTM2.OSTMnTT.BIT.OSTMnTT = 1;

    /* ---- OSTM compare register (CMP) setting ---- */
    OSTM2.OSTMnCMP.LONG =  (uint32_t)(- 1) ;

    /* Work in compare mode mode. */
    OSTM2.OSTMnCTL.BIT.OSTMnMD1 = 1;

    /* Don't use interrupts. */
    OSTM2.OSTMnCTL.BIT.OSTMnMD0 = 0;

    OSTM2.OSTMnTS.BIT.OSTMnTS = 1;  /* Start counting */

    time1 = OSTM2.OSTMnCNT.LONG;
    time2 = OSTM2.OSTMnCNT.LONG;
...
void PerformSetStartTime(uint32_t timer_no)
{
    ...
    {
        app_time[timer_no][START_TIME] = OSTM2.OSTMnCNT.LONG;
    }
...
void PerformSetEndTime(uint32_t timer_no)
{
    if (timer_no < TIMER_MAX)
    {
        app_time[timer_no][END_TIME] = OSTM2.OSTMnCNT.LONG;
    }
...
static uint32_t GetElapsedClockCount(uint32_t timer_no)
{
    ...
    if (timer_no < TIMER_MAX)
    {
        ...
        {
            /* Calculate the number of clocks from the start time to the end time */
            clock_count = ((0xFFFFFFFF - app_time[timer_no][START_TIME] + 1) + app_time[timer_no][END_TIME]) - overhead;
        }
    }
