...
static uint8_t frame_RAM[R_BCD_CAMERA_HEIGHT * R_BCD_CAMERA_WIDTH * 3] __attribute__ ((section("ImageWork_RAM")));
...
void sample_main(void)
{
    ...
    frame_RAM2 = &frame_RAM[R_BCD_CAMERA_HEIGHT * R_BCD_CAMERA_WIDTH * 1];
    frame_RAM3 = &frame_RAM[R_BCD_CAMERA_HEIGHT * R_BCD_CAMERA_WIDTH * 2];
...
/* Function : Resize Bilinear Fixed */
       ...
       /* Write the canny's result into the graphics buffer */
		...
		ret_val = R_DK2_Load(&g_drp_cropping_rgb[0],
				R_DK2_TILE_0 | R_DK2_TILE_2 | R_DK2_TILE_4,
				R_DK2_TILE_PATTERN_2_2_2, NULL, &cb_drp_finish, &drp_lib_id[0]);
		DRP_DRV_ASSERT(ret_val);
		...
		{
			/* Set the address of buffer to be read/write by DRP */
			param_b2rgb[tile_no/2].src = input_bufadr + (R_BCD_CAMERA_WIDTH * (R_BCD_CAMERA_HEIGHT / 3)) * (tile_no/2);
			param_b2rgb[tile_no/2].dst = output_bufadr + (R_BCD_CAMERA_WIDTH * (R_BCD_CAMERA_HEIGHT / 3)) * (tile_no/2);
		...
		//R_BCD_DRP_do_CroppResizeAI_RGB( (uint32_t)output_bufadr, (uint32_t)frame_RAM2, (uint32_t)frame_RAM3);
...
int R_BCD_DRP_do_CroppResizeAI_RGB ( uint32_t input_address, uint32_t crop_output_address , uint32_t resize_output_address ) {
	int16_t ret_val = 0;
	uint32_t i = 0;
		...
		while ( (R_BCD_DrpSetCropParam(i, &parm_crop_gry) != 0) & (i < R_BCD_DRP_CROP_NUM)) {
			if ( checker == (uint8_t)0 ) {
				...
				*************************************************************************/
				...
				parm_crop_rgb.dst = crop_output_address;    /* Address of output image. */
			  ...
			  *************************************************************************/
				...
				parm_resize_rgb.src = crop_output_address;       /* Address of input image. */
				parm_resize_rgb.dst = resize_output_address;    /* Address of output image. */
				parm_resize_rgb.src_width = parm_crop_gry.dst_width;   /* Input data width (Pixel) */
				parm_resize_rgb.src_height = parm_crop_gry.dst_height; /* Input data height (Pixel) */
			...
			/**************************************************************************
					Histogram Mode 1
			***************************************************************************/
			parm_histogram_rgb[0].src = resize_output_address;     /* Address of input image */
			parm_histogram_rgb[0].dst = (uint32_t)&histRGB_output_data[0];     /* Address of output image */
			...
			***************************************************************************/
			...
			parm_histogram_rgb[0].src = resize_output_address;            /* Address of input image */
			parm_histogram_rgb[0].dst = resize_output_address;            /* Address of output image */
			parm_histogram_rgb[0].width = R_BCD_AI_INPUT_X;                                /* The horizontal size (pixels) of image */
			parm_histogram_rgb[0].height = R_BCD_AI_INPUT_Y;                                     /* The vertical size (pixels) of image */
			...
			/* Preform AI */
			/***************************************/
			//ai_result = R_BCD_AI((uint8_t*)resize_output_address);
			i++;
		}
	}
	return ret_val;
